...
class Validate {
    ...
    public function name($name, $value, $required = true) {
        ...
        //Call the pattern method to validate name
        $pattern = '/^[a-zA-Z]*$/';
    ...
    public function phone($name, $value, $required = false) {
        ...
        if (preg_match('/^[[:alpha:]]$/', $name)) {
            $field->setErrorMessage('No letters.');
            return;
        }
    ...
    public function email($name, $value, $required = true) {
        ...
        // Check for special characters in local and domain
        if (!preg_match("/^[a-zA-Z@\.0-9]*$/",$local)) {
            $field->setErrorMessage('No special characters in username.');
            return;
        }
    ...
    public function zipcode($name, $value, $required = false) {
        $field = $this->fields->getField($name);
        ...
        // Call the pattern method to validate a zipcode
        $pattern = '/^\d{5}$/';
